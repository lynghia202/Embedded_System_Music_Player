...
static QueueHandle_t g_keypadQueue = NULL; // Nhan phim tu Task Quet -> Gui sang Task Logic
static QueueHandle_t g_lcdQueue = NULL;		// Gui tu Task Logic -> LCD

// Mutex: Bao ve tai nguyen dung chung
static SemaphoreHandle_t g_i2cMutex = NULL; // Bao ve bus I2C (Tranh Timer va Task LCD cung truy cap)
static SemaphoreHandle_t g_uartMutex = NULL; // Bao ve UART (Tranh gui lenh chong cheo len DFPlayer)
...
static void vPlayerLogicTask(void *pvParameters) {
    ...
    uint8_t needsDelayBeforeUpdate = 0; // Co hieu de giu man hinh thong bao (UX)
...
static void vKeypadScanTask(void *pvParameters) {
    ...
    for (;;) {
        ...
        if (xSemaphoreTake(g_keypadSemaphore, portMAX_DELAY) == pdPASS) {
            ...
            Keypad_Configure_For_Scan();// Cau hinh GPIO de quet ma tran 	 
            key = Keypad_Scan();	// Doc phim duoc bam
            Keypad_Configure_For_Interrupt(); // Cau hinh lai GPIO cho Ngat
            
            EXTI_ClearITPendingBit(EXTI_Line0 | EXTI_Line1 | EXTI_Line2 | EXTI_Line3); //Xoa co ngat
            EnableAllKeypadIRQ();
...
static void vLowPower_ConfigUnusedPins(void) {
    ...
    uint16_t UsedPins_B = GPIO_Pin_6 | GPIO_Pin_7;
...
static void prvSetupHardware(void) {
    ...
    RCC_APB2PeriphClockCmd(RCC_APB2Periph_ADC1 | RCC_APB2Periph_ADC2 | RCC_APB2Periph_SPI1 | RCC_APB2Periph_TIM1, DISABLE);
    RCC_APB1PeriphClockCmd(RCC_APB1Periph_TIM2 | RCC_APB1Periph_TIM3 | RCC_APB1Periph_TIM4 | RCC_APB1Periph_SPI2 | RCC_APB1Periph_USB | RCC_APB1Periph_DAC | RCC_APB1Periph_WWDG, DISABLE);
	
		// 3. Bat Clock cho cac ngoai vi can thiet
    RCC_APB2PeriphClockCmd(DFPLAYER_GPIO_RCC | RCC_APB2Periph_GPIOA | RCC_APB2Periph_GPIOB | DFPLAYER_USART_RCC | RCC_APB2Periph_AFIO, ENABLE);
